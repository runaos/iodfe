...
ifneq ($(BUILD_SERVER),0)
  TARGETS += $(B)/iodfeded$(FULLBINEXT)
endif

ifneq ($(BUILD_CLIENT),0)
  ifneq ($(USE_RENDERER_DLOPEN),0)
    TARGETS += $(B)/iodfengine$(FULLBINEXT) $(B)/renderer_opengl1_$(SHLIBNAME)
    ifneq ($(BUILD_CLIENT_SMP),0)
      TARGETS += $(B)/renderer_opengl1_smp_$(SHLIBNAME)
    endif
  else
    TARGETS += $(B)/iodfengine$(FULLBINEXT)
    ifneq ($(BUILD_CLIENT_SMP),0)
      TARGETS += $(B)/iodfengine-smp$(FULLBINEXT)
    endif
  endif
endif
...
ifneq ($(USE_RENDERER_DLOPEN),0)
$(B)/iodfengine$(FULLBINEXT): $(Q3OBJ) $(LIBSDLMAIN)
	$(echo_cmd) "LD $@"
...
$(B)/iodfengine$(FULLBINEXT): $(Q3OBJ) $(Q3ROBJ) $(Q3POBJ) $(LIBSDLMAIN)
	$(echo_cmd) "LD $@"
	$(Q)$(CC) $(CLIENT_CFLAGS) $(CFLAGS) $(CLIENT_LDFLAGS) $(LDFLAGS) \
		-o $@ $(Q3OBJ) $(Q3ROBJ) $(Q3POBJ) \
		$(LIBSDLMAIN) $(CLIENT_LIBS) $(RENDERER_LIBS) $(LIBS)

$(B)/iodfengine-smp$(FULLBINEXT): $(Q3OBJ) $(Q3ROBJ) $(Q3POBJ_SMP) $(LIBSDLMAIN)
	$(echo_cmd) "LD $@"
...
ifneq ($(BUILD_CLIENT),0)
	$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/iodfengine$(FULLBINEXT) $(COPYBINDIR)/iodfengine$(FULLBINEXT)
  ifneq ($(USE_RENDERER_DLOPEN),0)
...
ifneq ($(BUILD_CLIENT_SMP),0)
  ifneq ($(USE_RENDERER_DLOPEN),0)
	...
	$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/iodfengine-smp$(FULLBINEXT) $(COPYBINDIR)/iodfengine-smp$(FULLBINEXT)
  endif
endif

ifneq ($(BUILD_SERVER),0)
	@if [ -f $(BR)/iodfeded$(FULLBINEXT) ]; then \
		$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/iodfeded$(FULLBINEXT) $(COPYBINDIR)/iodfeded$(FULLBINEXT); \
	fi
endif
...
dist:
	rm -rf iodfengine-$(VERSION)
	svn export . iodfengine-$(VERSION)
	tar --owner=root --group=root --force-local -cjf iodfengine-$(VERSION).tar.bz2 iodfengine-$(VERSION)
	rm -rf iodfengine-$(VERSION)
